...
@implementation SWDBDataBaseModel

#pragma mark - override method
+ (void)initialize
{
    if (self != [SWDBDataBaseModel self]) {
        [self createTable];
    }
}
...
{
    __block BOOL res = NO;
    SWDBDataBase *jkDB = [SWDBDataBase shareInstance];
...
+ (BOOL)createTable
{
    FMDatabase *db = [FMDatabase databaseWithPath:[SWDBDataBase dbPath]];
    if (![db open]) {
        NSLog(@"数据库打开失败!");
        return NO;
    }
...
{
    //判断是否是JKBaseModel的子类
    for (SWDBDataBaseModel *model in array) {
        if (![model isKindOfClass:[SWDBDataBaseModel class]]) {
            return NO;
        }
    }
...
+ (NSArray *)findAll
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            SWDBDataBaseModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
